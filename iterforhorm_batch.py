...
# Save directory 
save_dir = Path('/scratch/tshekhaw')

# Batch size is set to 100000000, can change later
BATCH_SIZE = 10000000
lst = []
row = 0
batch_number = 0
                        ...
                        for o in endmt_bgdper:
                            for p in gd_vaginal_insertion_gdp:
                                ...
                                if len(lst) >= BATCH_SIZE:
                                    # Save current batch
                                    batch_file = save_dir / f'hormonal_iud_scores_batch_{batch_number}.npz'
                                    np.savez_compressed(batch_file, data=lst)
                                    
                                    # Reset list and increment batch number
                                    lst = []
                                    batch_number += 1
                                    
                                    # Optional: Print progress
                                    print(f"Saved batch {batch_number}, processed {row} rows")

# Save any remaining data in the final batch
if lst:
    batch_file = save_dir / f'hormonal_iud_scores_batch_{batch_number}.npz'
    np.savez_compressed(batch_file, data=lst)
    print(f"Saved final batch {batch_number}, processed {row} total rows")

# Function to load and combine all batches for analysis
def load_all_batches(directory):
    all_scores = []
    for npz_file in sorted(directory.glob('hormonal_iud_scores_batch_*.npz')):
        with np.load(npz_file) as data:
            batch_data = data['data']
            scores = [item[1] for item in batch_data]
            all_scores.extend(scores)
    return all_scores

# Load all batches and generate statistics
scores = load_all_batches(save_dir)
...
# Optional: Clean up batch files after analysis
def cleanup_batch_files(directory):
    for npz_file in directory.glob('hormonal_iud_scores_batch_*.npz'):
        npz_file.unlink()
